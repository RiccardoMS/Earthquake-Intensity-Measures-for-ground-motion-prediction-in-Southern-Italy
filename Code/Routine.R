...
#Azi        :=source-site azimut,still no clues about that
...
points(StLon,StLat,type = 'p',col='blue',cex=0.6)
...
qqnorm(log10(FAS_EW),ylim=c(-5,2))                     #also here, gaussianity not assured, very heavy tails
qqline(log10(FAS_EW))
...
detach(DATA)
for(i in 1:length(as.vector(DATA$Dipo))){
  if(as.vector(DATA$Dipo)[i]>200)
    DATA<-DATA[-i,] }
attach(DATA)
...
UsableVS30=ifelse(is.na(VS30),VS30_WA,VS30)
DATA<-cbind(DATA,UsableVS30)
remove(UsableVS30)
table(is.na(UsableVS30))
UsableVS30[is.na(UsableVS30)==T]=1500        #come li trattiamo?
...
#to evaluate:possible transofrmation of DIpo in sqrt(DIpo)
...
#LogY=function(ML,LogY)  (experimental)
library('plot3D')
...
y <- Dipo
z <- LogY
# Compute the linear regression (z = ax +bx^2 +cx*log10(y) + dy+ e)
...
scatter3D(x, y, z, pch = 18, cex = 0.01, 
          ...
          xlab = "ML", ylab = "Dipo", zlab = "LogY",  
          surf = list(x = x.pred, y = y.pred, z = z.pred,  
                      facets = NA, fit1 = fit1points), main = "RegPlane")
scatter3D(x, y, z, pch = 18, cex = 0.01, 
          theta = 0, phi = 20, ticktype = "detailed",
          xlab = "ML", ylab = "Dipo", zlab = "LogY",  
          surf = list(x = x.pred, y = y.pred, z = z.pred,  
